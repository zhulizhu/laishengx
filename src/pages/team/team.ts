...
export class TeamPage {
      ...
      if(res.status==1){
        ...
        for (let i = 0; i < this.door.length; i++) {
          ...
          for (let j = 0; j < this.itemA.length; j++) {
            if (this.itemA[j].storename == this.door[i].storename) {
  ...
  // turnDate(e){
  //   let time=new Date(parseInt(e) * 1000).toLocaleString().replace(/:\d{1,2}$/,' ');
  ...
  teamDtail(id,week,salesreal,total) {


    if (parseInt(total)>parseInt(salesreal)&&parseInt(salesreal)>=0) {
      this.navCtrl.push('HelpdetailPage',{id:id,week:week,course_type:2});
    }else {
